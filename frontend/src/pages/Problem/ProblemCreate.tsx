...
export default function ProblemCreate() {
  ...
  const onFinish = async (values: any) => {
    setLoading(true);
    try {
      // Defensive deep clone of signatures — each element must be a real object
      ...
      if (emptyLangIdx !== -1) {
        ...
        console.error('[ProblemCreate] Signature with empty language detected at index', emptyLangIdx);
      ...
      // eslint-disable-next-line no-console
      console.error('[ProblemCreate] onFinish error:', err);
      if (err && typeof err === 'object' && err.stack) {
        // eslint-disable-next-line no-console
        console.error('[ProblemCreate] stack:', err.stack);
      }
  ...
  const onFinishFailed = (errorInfo: any) => {
    // eslint-disable-next-line no-console
    console.error('[ProblemCreate] Form validation failed:', errorInfo);
  ...
  return (
    <div>
      ...
      <Form form={form} layout="vertical" onFinish={onFinish} onFinishFailed={onFinishFailed} style={{ width: '100%' }} initialValues={{ difficulty: 'medium', time_limit: 1000, memory_limit: 256, compare_mode: 'exact' }}>
