    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f2f5' }}>
      <Card style={{ width: 420 }}>
        ...
        <Form onFinish={onFinish} size="large" initialValues={{ role: 'student' }}>
          <Form.Item name="username" rules={[{ required: true, message: '请输入用户名' }, { min: 3, max: 50, message: '3-50个字符' }]}>
          ...
          <Form.Item name="password" rules={[{ required: true, message: '请输入密码' }, { min: 8, message: '至少8个字符' }]}>
            <Input.Password prefix={<LockOutlined />} placeholder="密码（至少8位，含大小写字母和数字）" />
          </Form.Item>
